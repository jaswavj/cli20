...
try {
    ...
    while(rs.next()) {
        ...
        // Get batch ID for this product — prefer in-stock batch, fall back to latest
        int batchId = 0;
        try {
            ...
            batchPs.close();
            if(batchId == 0) {
                batchPs = conn.prepareStatement("SELECT id FROM prod_batch WHERE product_id = ? ORDER BY id DESC LIMIT 1");
                batchPs.setInt(1, prodId);
                batchRs = batchPs.executeQuery();
                if(batchRs.next()) {
                    batchId = batchRs.getInt("id");
