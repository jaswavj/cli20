...
function addProduct() {
    ...
    const row = `
        <tr id="row${count}" data-batch-id="${productBatch}" data-commission="${productCommission}" data-subtotal="${productSubtotal}" data-discount="0" data-total="${total}" data-qty="${actualQty}" data-price="${price}" class="bill-item-row" style="cursor: pointer;">
            <td style="width: 5%;">${count}</td>
            <td style="width: 10%;" data-id="${productId}">${code}</td>
            <td style="width: 30%;" data-name="${name}">${name}</td>
...
function saveBill() {
    ...
    // Tax bill always ON (toggle removed)
    let isTaxBill = 1;
...
function addOrderItemToBill(item) {
    ...
    const row = `
        <tr id="row${count}" class="item-row bill-item-row" data-batch-id="${batchId}" data-commission="0" data-subtotal="${item.total}" data-discount="0" data-total="${item.total}" data-qty="${item.qty}" data-price="${item.price}">
            <td style="width: 5%;">${count}</td>
            <td style="width: 10%;" data-id="${item.prodId}">${item.code || ''}</td>
            <td style="width: 30%;">${item.prodName}</td>
...
function newBill() {
    ...
    // Tax bill always ON (toggle removed – no element to reset)
...
function addProductToBillTable(product) {
    ...
    const row = `
        <tr id="row${count}" class="bill-item-row" style="cursor: pointer;" data-batch-id="${product.batchId || 0}" data-commission="${productCommissionPerUnit}" data-subtotal="${productSubtotal}" data-discount="${productDiscount}" data-total="${productTotal}" data-qty="${product.qty}" data-price="${parseFloat(product.price)}">
            <td style="width: 5%;">${count}</td>
            <td style="width: 10%;" data-id="${product.productId}">${product.code}</td>
            <td style="width: 30%;" data-name="${product.productName}">${product.productName}</td>
